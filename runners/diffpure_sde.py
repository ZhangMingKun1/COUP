...
class RevVPSDE(torch.nn.Module):
	def __init__(self, args, model, score_type='guided_diffusion', beta_min=0.1, beta_max=20, N=1000,
				img_shape=(3, 256, 256), model_kwargs=None, mode='sde', cls=None):
		...
		if return_type == 'drift':
			...
			if self.args.Guide:
				with torch.enable_grad():
					xt = x.detach().view(x.size(0), self.img_shape[0], self.img_shape[1], self.img_shape[2])
					
					xt.requires_grad = True
					logits = self.cls((xt + 1) * 0.5)
					logits = torch.softmax(logits, dim=1)

					prob = logits.max(axis=-1)[0]
					log_prob = torch.log(prob)
					log_prob = log_prob.requires_grad_()
					log_prob.backward(torch.ones_like(log_prob), retain_graph=True)
					grad_prob = xt.grad
			...
			if self.args.Guide:
				drift = drift - diffusion[:, None] ** 2 * (score + self.args.lmd_guide * grad_prob.view(x.size(0), -1))
			else:
...
class RevGuidedDiffusion(torch.nn.Module):
	def __init__(self, args, config, device=None):
		...
		self.img_shape = img_shape
		self.model = model
		# self.rev_vpsde = RevVPSDE(args=self.args, model=model, score_type=args.score_type, img_shape=img_shape,
		# 						model_kwargs=None, mode=self.args.mode, cls=cls).to(self.device)
	...
	def image_editing_sample(self, img, bs_id=0, tag=None, mode='sde', cls=None):
		with amp.autocast(enabled=False):
			self.rev_vpsde = RevVPSDE(args=self.args, model=self.model, score_type=self.args.score_type, img_shape=self.img_shape,
									model_kwargs=None, mode=self.args.mode, cls=cls).to(self.device)
			self.betas = self.rev_vpsde.discrete_betas.float().to(self.device)
			...
			for it in range(self.args.sample_step):
				...
				if self.args.use_bm:
					with amp.autocast(enabled=False):
						bm = torchsde.BrownianInterval(t0=t0, t1=t1, size=(batch_size, state_size), device=self.device, dtype=torch.float32)
						xs_ = torchsde.sdeint_adjoint(self.rev_vpsde, x_, ts, method='euler', bm=bm, dtype=torch.float32)
				else:
					with amp.autocast(enabled=False):
						xs_ = torchsde.sdeint_adjoint(self.rev_vpsde, x_.float(), ts, method='euler', dtype=torch.float32)
			...
			return torch.cat(xs, dim=0).clamp(-1., 1.)
