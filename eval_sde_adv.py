...
class SDE_Adv_Model(nn.Module):
	...
	def get_purified_img(self):
		return self.purified_img
	...
	def forward(self, x):
		...
		with amp.autocast(enabled=False):
			x_re = self.runner.image_editing_sample((x.float() - 0.5) * 2, bs_id=counter, tag=self.tag, mode=self.args.mode, cls=self.classifier)
...
def eval_autoattack(args, config, model, x_val, y_val, adv_batch_size, log_dir):
	...
	if attack_version == 'standard':
		attack_list = ['square', 'apgd-ce', 'apgd-t', 'fab-t']
...
def robustness_eval(args, config):
	...
	log_dir = os.path.join(args.image_folder, args.classifier_name, middle_name,
						'seed' + str(args.seed), 'data' + str(args.data_seed) + 'Guide' + str(args.Guide) + 'Guide_type' + str(args.Guide_type) + 'temp' + str(args.temp) + 'lmd_Guide' + str(args.lmd_guide) + 'decay_Guide' + str(args.decay_guide) + 't' + str(args.t))
...
def parse_args_and_config():
	...
	parser.add_argument('--t', type=int, default=40, help='Sampling noise scale')
	...
	parser.add_argument('--classifier_guide_name', type=str, default='Wang2023Better_WRN-28-10', help='which classifier to use')
	...
	parser.add_argument('--lmd_guide', type=float, default=1.)
	parser.add_argument('--mode', type=str, default='ga', choices=['diffPure', 'sde', 'de', 'ode', 'ld', 'ga'])
