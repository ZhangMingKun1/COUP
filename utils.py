...
def get_image_classifier(classifier_name):
	...
	if 'imagenet' in classifier_name:
		...
		if 'wideresnet-28-10' in classifier_name and 'true' not in classifier_name:
			print('using cifar10 wideresnet-28-10...')
			model = load_model(model_name='Standard', dataset='cifar10', threat_model='Linf')  # pixel in [0, 1]
		elif 'wideresnet-28-10_true' in classifier_name:
			print('using cifar10 wideresnet-28-10_true...')
			model_path = '/data/users/zhangmingkun/Rectified-Rejection/trained_models/CIFAR-10/PGDAT_wideresnet-28-10BN_adaptiveT1.0_selfreweightCalibrate_SGconW_seed0/model_best.pth'
			model = load_model(model_name='Standard', dataset='cifar10', threat_model='Linf')
			model.load_state_dict(update_state_dict(torch.load(model_path)['state_dict'], idx_start=7))
			model.eval()

		elif 'wang2023Better-wrn-28-10' in classifier_name:
			print('using cifar10 Wang2023Better_WRN-28-10 ...')
			# model = load_model(model_name='Wang2023Better_WRN-28-10', dataset='cifar10', threat_model='Linf', model_dir='/data/users/zhangmingkun/DiffPure-master/models/')
			model = load_model(model_name='Wang2023Better_WRN-28-10', dataset='cifar10', threat_model='L2', model_dir='/data/users/zhangmingkun/DiffPure-master/models/')
